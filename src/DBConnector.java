...
public class DBConnector {
    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(
            ...
            List<Paper> XMLcontent = DomParser.grabXMLContent();
            for(Paper p : XMLcontent) {
                String insertString1 = "insert into CS7340_Lab1.papers values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                String insertString2 = "insert ignore into CS7340_Lab1.authors (`authorName`) values(?)";
                String selectString1 = "select authorId from CS7340_Lab1.authors where authorName = ?";
                String insertString3 = "insert into CS7340_Lab1.coauthors values(?,?)";

                // insert paper into papers table
                try (PreparedStatement insertPaper = conn.prepareStatement(insertString1))
                {
                ...
                try (PreparedStatement insertAuthor = conn.prepareStatement(insertString2))
                {
                ...
                try (PreparedStatement selectAuthorId = conn.prepareStatement(selectString1);
                     PreparedStatement insertCoauthor = conn.prepareStatement(insertString3))
                {
                    // loop through authors for current paper
                    for(String authorName : p.authors) {
                        // gets Id of author
            ...
            // TODO: write queries 1-4 (and accept user input)
            // example for how to query data from mySQL and write it to the terminal

            /*
            String selectSQL = "select * from Lab1.papers";
            ResultSet rs = stmt.executeQuery(selectSQL);
            while(rs.next()){
                String mdate = rs.getString("mdate");
                String key = rs.getString("paperKey");
